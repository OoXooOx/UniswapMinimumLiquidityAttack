...
contract UniswapERC20 is IUniswapERC20 {
    ...
    string public constant name = 'Uniswap LPs';
    string public constant symbol = 'Uniswap-LP';
...
contract UniswapPair is IUniswapPair, UniswapERC20 {
    ...
    function setDevFee(uint32 _devFee) external {
        require(_devFee > 0, "UniswapPair: lower then 0");
        require(msg.sender == factory, 'UniswapPair: FORBIDDEN');
        require(_devFee <= 500, 'UniswapPair: FORBIDDEN_FEE');
        devFee = _devFee;
    }
...
contract UniswapFactory is IUniswapFactory {
    ...
    bytes32 public INIT_CODE_HASH = keccak256(abi.encodePacked(type(UniswapPair).creationCode));
    ...
    function setDevFee(address _pair, uint8 _devFee) external {
        require(msg.sender == feeToSetter, 'Uniswap: FORBIDDEN');
        require(_devFee > 0, 'Uniswap: FORBIDDEN_FEE');
        UniswapPair(_pair).setDevFee(_devFee);
    }
    
    function setSwapFee(address _pair, uint32 _swapFee) external {
        require(msg.sender == feeToSetter, 'Uniswap: FORBIDDEN');
        UniswapPair(_pair).setSwapFee(_swapFee);
    }


    function getBytecode() external pure returns(bytes memory bytecode) {
        bytecode=type(UniswapPair).creationCode;
    }
}