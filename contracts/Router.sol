...
library UniswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'4764d9a8fbee62630d53117d5da0f92b894f9f2722112645427fec4e2effb3b2' // init code hash
            ))));
    }

    function getSwapFee(address factory, address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = IUniswapPair(pairFor(factory, tokenA, tokenB)).swapFee();
    }
    ...
    //                                           5                 2 token     2 weth
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) { // 5 
        require(amountA > 0, 'UniswapLibrary: INSUFFICIENT_AMOUNT'); //true
        require(reserveA > 0 && reserveB > 0, 'UniswapLibrary: INSUFFICIENT_LIQUIDITY'); // true
        amountB = amountA.mul(reserveB) / reserveA; // magic  5 *2 /2 = 5 
    }
...
contract UniswapRouter02 is IUniswapRouter02, Ownable {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = UniswapLibrary.quote(amountADesired, reserveA, reserveB); // 5
            if (amountBOptimal <= amountBDesired) { // 5  <= 2  false
                require(amountBOptimal >= amountBMin, 'UniswapV2Router: INSUFFICIENT_B_AMOUNT'); // 0.5*10**18 >=0.5*10**18 false
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = UniswapLibrary.quote(amountBDesired, reserveB, reserveA); // 0.14*17/17=0.14
                assert(amountAOptimal <= amountADesired);//0.14<=1 true
                require(amountAOptimal >= amountAMin, 'UniswapV2Router: INSUFFICIENT_A_AMOUNT'); // 0.14>=1
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = UniswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.swapFee());
            }
            if (swapFeeReward != address(0)) {
    ...
    {
        return UniswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
    }
    ...
    {
        return UniswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, swapFee);
    }
